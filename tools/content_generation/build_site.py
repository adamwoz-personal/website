...
def build_chat() -> str:
    ...
    <section class="card chat-card">
      ...
      <p class="chat-footnote">Runs on a small self-hosted inference service with per-IP rate limits and a daily token budget to bound costs. For abuse triage, requests log a hashed IP, token counts, and the first 64 characters of the message body — so avoid pasting anything you would not want retained. See <a href="/adam/philosophy/#agentic-ai">the philosophy essay</a> for the design thinking.</p>
    </section>
